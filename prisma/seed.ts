/**
 * Demo seed — build step 1 (docs/spec/README.md §6, docs/spec/seed-data.md).
 *
 * Produces a fully fictional dataset: no real people, no real transactions.
 * Idempotent by wipe-then-insert; DEV/TEST databases only — it refuses to run
 * against NODE_ENV=production.
 *
 * Run: pnpm/npm run db:seed   (needs DATABASE_URL; see .env.example)
 */
import { db } from "../lib/db";
import { auth } from "../lib/auth";
import { GROUPS, CATEGORIES } from "./seed/taxonomy";
import { MAKES, MODEL_GROUPS, GENERATIONS } from "./seed/vehicles";
...
async function createLogin(
  ...
  await db.account.create({
    data: {
      ...
      password: await ctx.password.hash(DEMO_PASSWORD),
    },
  });
  return user;
}
...
let orderCounter = 0;
const code = (prefix: string, n: number) =>
...
async function main() {
  ...
  const categoryBySlug = await seedTaxonomy();
  const generationBySlug = await seedVehicles();

  // --- People ---------------------------------------------------------------
  // Staff: one login per founder (attributable) — matches the committed staff script.
  await createLogin("lucy@ivo.example", "Lucy (staff)", "staff");
  await createLogin("ivo@ivo.example", "Ivo (staff)", "staff");

  // Buyers (self-registered shape: User + Buyer profile with a delivery address).
  ...
  // Sellers: 4 profiles, staff-created. One (index 0) has a provisioned login and
  // is the demo target that must exercise every seller-center surface.
  ...
  const sellerDefs = [
    { displayName: "Sofia Auto Dismantlers", city: "Sofia", userId: loginSellerUser.id },
  ...
  for (const p of listingPlan) {
    ...
    const part = await db.part.create({
      data: {
        internalCode: code("PRT", partCounter),
    ...
    const listing = await db.listing.create({
      data: {
        internalCode: code("LST", listingCounter),
        partId: part.id,
  ...
  // Add a fresh placed + a shipped + a cancelled by reusing published listings.
  ...
  for (const o of orderPlan) {
    ...
    const order = await db.order.create({
      data: {
        internalCode: code("ORD", orderCounter),
        buyerId: buyer.id,
    ...
    // One PENDING cancellation request on the confirmed order (exercises the
    // seller-center "approve cancellation" write action + admin pending list).
    ...
    // Notification rows for the events that have happened (in-app only).
    ...
    // Seller (has a login) gets the placed notification.
  ...
  for (let i = 0; i < threadTargets.length; i++) {
    ...
    await db.message.create({
      data: {
        ...
        readAt: i === 0 ? new Date() : null, // thread 0 read, threads 1-2 unread
      },
    });
    if (i === 0) {
      await db.message.create({
  ...
  console.log("Login seller: yard.sofia@example.com");
  console.log("Buyers: maria.buyer@example.com, georgi.buyer@example.com, elena.buyer@example.com");
}
